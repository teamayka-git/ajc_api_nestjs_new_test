...
export class AccountHeadService {
    ...
    async create(dto: AccountHeadCreateDto, _userId_: string) {
      ...
      async edit(dto: AccountHeadEditDto, _userId_: string) {
        var dateTime = new Date().getTime();
        const transactionSession = await this.connection.startSession();
        transactionSession.startTransaction();
        try {
          var result = await this.accountHeadModel.findOneAndUpdate(
            {
              _id: dto.accountheadId,
            },
            {
              $set: {
                _code: dto.code,
  ...
  async status_change(dto: AccountHeadStatusChangeDto, _userId_: string) {
    var dateTime = new Date().getTime();
    const transactionSession = await this.connection.startSession();
    transactionSession.startTransaction();
    try {
      var result = await this.accountHeadModel.updateMany(
        {
          _id: { $in: dto.accountheadIds },
        },
        {
          $set: {
            _updatedUserId: _userId_,
  ...
  async list(dto: AccountHeadListDto) {
    var dateTime = new Date().getTime();
    const transactionSession = await this.connection.startSession();
    transactionSession.startTransaction();
    try {
      ...
      if (dto.accountheadIds.length > 0) {
        var newSettingsId = [];
        dto.accountheadIds.map((mapItem) => {
      ...
      var result = await this.accountHeadModel
      ...
      if (dto.screenType.includes(0)) {
        ...
        var resultTotalCount = await this.accountHeadModel
          .aggregate(arrayAggregation)
          .session(transactionSession);
  ...
  async checkNameExisting(dto: CheckNameExistDto) {
    ...
    transactionSession.startTransaction();
    try {
      var resultCount = await this.accountHeadModel
        .count({
          _name: dto.value,
