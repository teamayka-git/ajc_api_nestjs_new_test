...
const descriptionListScreenTypeForList =
  '0-total documents count, 100-user details, 103-root cause details, 104-created user details, 105-sales return items, 106-sub category only if sales return items exist,107 -  user details under[100] global gallery ,108 -  created user details under[104] global gallery, 109 - sales return items under[105] sub category, 110 - shop details, 111 - shop under[110] shop user details';
...
class SalesReturnCreateListItems {
  @IsString()
  @ApiProperty({})
  orderId: string;
  @IsString()
...
class SalesReturnCreateList {
  ...
  @IsArray()
  @ApiProperty({ type: [SalesReturnCreateListItems] })
  @ValidateNested({ each: true })
  @Type(() => SalesReturnCreateListItems)
  arraySalesReturnItems: SalesReturnCreateListItems[];
...
export class SalesReturnCreateDto {
  @IsNumber()
  @ApiProperty({})
  isOrderComplete: number;

  @IsArray()
  @ApiProperty({ type: [SalesReturnCreateList] })
  @ValidateNested({ each: true })
  @Type(() => SalesReturnCreateList)
  salesReturns: SalesReturnCreateList[];
}
...
export class SalesReturnListDto {
  ...
  @IsNumber()
  @ApiProperty({})
  salesReturnDateStartDate: number;

  @IsNumber()
  @ApiProperty({})
  salesReturnDateEndDate: number;

  @IsArray()
  @ApiProperty({ type: [String] })
  salesReturnUids: string[];
...
export class SalesReturnStatusChangeDto {
  @IsArray()
  @ApiProperty({ type: [String] })
  salesReturnIds: string[];
