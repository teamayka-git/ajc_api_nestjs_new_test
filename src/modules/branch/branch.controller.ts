import { Controller, Get, Post, Body, Patch, Param, Delete, Request, Put, UseGuards, UseInterceptors, UploadedFiles } from '@nestjs/common';
...
export class BranchController {
  ...
  create(@Body() dto: BranchCreateDto,@Request() req, @UploadedFiles() file) {
    return this.branchService.create(dto,req["_userId_"],file == null ? {} : JSON.parse(JSON.stringify(file)));
  }
  ...
  edit(@Body() dto: BranchEditDto,@Request() req, @UploadedFiles() file) {
    return this.branchService.edit(dto,req["_userId_"],file == null ? {} : JSON.parse(JSON.stringify(file)));
  }
  @Delete()
  @Roles(GuardUserRole.SUPER_ADMIN)
