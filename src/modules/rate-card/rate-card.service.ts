...
export class RateCardService {
    ...
    try{
        ...
        var result1 = await this.rateCardsModel.insertMany(arrayToStates, {
    ...
    try{
        ...
        if (dto.rateCardIds.length > 0) {
          ...
          if (dto.subCategoryIds.length > 0) {
            ...
            arrayAggregation.push({ $match: { _subCategoryId: { $in: newSettingsId } } });
          }
    
        switch(dto.sortType){
          case 0: arrayAggregation.push({ $sort: { _id: dto.sortOrder } });              break;
          case 1:arrayAggregation.push({ $sort: { _status: dto.sortOrder } });               break;
          case 2: arrayAggregation.push({ $sort: { _percentage: dto.sortOrder } });               break;
          case 3: arrayAggregation.push({ $sort: { _subCategoryId: dto.sortOrder } });               break;
